...
export default function LearningDeskPage() {
  ...
  const getBookedCourses = async () => {
    try {
      ...
      if (courseArr.length > 0) {
        console.log("Logging course array")
  ...
  useEffect(() => {
    if (currentCourse && currentCourse.courseId.modulesIncluded.length > 0) {
      ...
      currentCourse.courseId.modulesIncluded.map((mod) => {
        ...
        const newData = [...currentModulesDate, {
          name: mod.name,
          start: modStartDate,
          end: modEndDate,
          duration: mod.noOfDays,
        }];
        setCurrentModulesDate(newData);
  ...
  useEffect(() => {
    ...
    setCurrentLernDay(0);
    if (currentCourse && currentModulesDate.length) {
                    ...
                    <section className="courseDetails">
                      ...
                      {course.courseId.modulesIncluded.length > 0 ? (
                        <ReactSpeedometer
                          ...
                          currentValueText={`${currentLernDay} ${currentLernDay > 1
                              ? intl.formatMessage({
                                defaultMessage: "days",
                                id: "days",
                              })
